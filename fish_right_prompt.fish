...
function prompt_arrow_right_close --argument-name BG FG END -d "close the previous fishline segment"
	set __octofish_PRE_CLOSE " "
	set __octofish_CLOSE "\uE0B2"
	set __octofish_POST_CLOSE " "

	if set -q __octofish_RIGHT_BCOLOR
		printf $__octofish_PRE_CLOSE
		set_color -b $__octofish_RIGHT_BCOLOR
	else
		set_color -b normal
	end
	set_color "$BG"
	printf $__octofish_CLOSE
	
	set_color -b $BG $FG
	if [ "$END" != True ]
		printf "$__octofish_POST_CLOSE"
	end
	set -g __octofish_RIGHT_BCOLOR $BG
end
